...
def process_video(
    ...
    if dataset=='ucf':
        video_captions, total_frames = load_video_captions_ucf(captions_dirs, video_name)
    elif dataset=='xd':
        video_captions, total_frames = load_video_captions_xd(captions_dirs, video_name)
    ...
    for batch_start_frame in tqdm(
        ...
        if text_list:
            ...
            file_names.extend(
                build_file_names(
                    caption_to_frame_idxs, text_list, video_name, frame_interval
                )
            )
...
def load_video_captions_ucf(captions_dirs: List[str], video_name: str):
...
def load_video_captions_xd(captions_dirs: List[str], video_name: str):
    video_captions = defaultdict(dict)
    captions_dir = Path(captions_dirs)

    video_caption_txt = captions_dir / f"{video_name}.txt"
    output_caption_json = captions_dir / f"{video_name}.json"
    parse_frame_caption_file(video_caption_txt, output_json_path=output_caption_json)
...
def build_file_names(caption_to_frame_idxs, text_list, video_name, frame_interval):
    file_names = []
...
def main(
    ...
    if dataset == 'ucf':
        ...
        video_list = load_video_xd('./list/xd_CLIP_rgbtest.csv')
...
def parse_args():
    ...
    parser.add_argument("--captions_dirs", default='/home/yeogeon/YG_main/diffusion_model/VAD_dataset')
    parser.add_argument("--output_dir", default='/home/yeogeon/YG_main/diffusion_model/VAD_dataset')
...
if __name__ == "__main__":
    args = parse_args()
    captions_dirs = args.captions_dirs
