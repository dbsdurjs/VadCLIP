...
SAVE_DIR         = '../VAD_dataset/UCF-Crimes/UCF_Crimes/refine_description/ucf_output_refine_2'
SAVE_DIR_META         = '../VAD_dataset/UCF-Crimes/UCF_Crimes/refine_description/ucf_meta_refine_2'
...
@torch.no_grad()
def apply_soft_matching_always(
    img_emb: torch.Tensor,                    # (N,512)
    txt_emb: torch.Tensor,                    # (M,512)
    global_floor: float = 0.29,
    mad_coeff: float = 0.5,
    temperature: float = 0.06,
    topk: int = 16,
    band_window: int = 8,
    time_prior_sigma: float = 4.0,
):
    # 참고: threshold_T와 hard_video_score는 로그용으로만 유지
    ...
    return {
        ...
        "threshold_T": torch.tensor(T, device=img_emb.device), # 로그용
        "hard_video_score": torch.tensor(hard_vs, device=img_emb.device),  # 비교지표(로그)
    }
...
def run_softmatch_with_x264_key(
    ...
    for img_p in img_paths:
        ...
        try:
            ...
            out = apply_soft_matching_always(
                img_emb, txt_emb,
            ...
            np.save(os.path.join(save_dir, f"{stem_img}_refine_2.npy"), # 1번 방법 - 평균 clip score 미만 soft match(_refine_1), 2번 방법 - 모두 soft match(_refine_2)
