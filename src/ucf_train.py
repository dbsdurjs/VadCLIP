    ...
    for e in range(args.max_epoch):
        ...
        with tqdm(total=min(len(normal_loader), len(anomaly_loader)), desc=f"Epoch {e+1}/{args.max_epoch}") as pbar:
            for i in range(min(len(normal_loader), len(anomaly_loader))):
                ...
                text_features, logits1, logits2, c_visual_feat, c_caption_feat, caption_logits, visual_logits  = model(visual_features, cap_features, None, prompt_text) # edit idea6-3
                ...
                loss = loss1 + loss2 + loss3 + loss_caption + loss_visual
